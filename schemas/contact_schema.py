...
class Contact(BaseModel):
    ...
    verification_level: str = Field(description="'verified', 'apollo_verified', 'inferred', or 'not_found'", default="not_found")
    source_url: Optional[str] = Field(description="URL where the contact was found", default=None)
    source_type: str = Field(description="E.g., 'linkedin_search', 'company_website', 'apollo', or 'none'", default="none")
    contact_confidence: int = Field(description="Confidence (0-100) that this is the correct decision maker", default=0)
    apollo_id: Optional[str] = Field(description="Unique Apollo profile ID", default=None)
